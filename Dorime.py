...
class UI:
    ...
    def Uran(event):  # Уран
        ...
        text = Text(info, bg='black', fg='white', borderwidth=0, width=700, height=300, font='Arial 10', wrap='word',
                    state='normal')  # Задаем текст  с инфой о планете
        text.insert(1.0,
                    'Уран удален от Солнца на расстояние в 19 раз большее, чем Земля,'
                    'поэтому получает очень мало тепла.')
        text.insert(5.1,
...
class Planet(object):
    ...
    def bounds(self):
        # Возвращает координаты границ объекта, в зависимости, от введённых значений х и у
...
path_iter_mercury = circular_path1(sun_obj.x, sun_obj.y, orbital_radius_mercury, CIRCULAR_PATH_INCR)
path_iter_venus = circular_path2(sun_obj.x, sun_obj.y, orbital_radius_venus, CIRCULAR_PATH_INCR)
path_iter_earth = circular_path3(sun_obj.x, sun_obj.y, orbital_radius_earth, CIRCULAR_PATH_INCR)
